...
async fn cults3d_status_handler(
    ...
    Ok(Json(commands::Cults3dStatus { creds_present, asset_host_configured, enabled, today_count }))
}
...
async fn sketchfab_status_handler(
    ...
    .bind(s.inner.project_id).bind(&today).fetch_one(&s.inner.pool).await.unwrap_or(0);
    Ok(Json(commands::SketchfabStatus { creds_present, enabled, sell_on_store, today_count }))
}
...
async fn gumroad_status_handler(
    ...
    .bind(s.inner.project_id).bind(&today).fetch_one(&s.inner.pool).await.unwrap_or(0);
    Ok(Json(commands::GumroadStatus { creds_present, enabled, today_count }))
}
...
async fn mmf_status_handler(
    ...
    Ok(Json(commands::MmfStatus { creds_present, enabled, sell_paid, today_count, client_id, oauth_user_id }))
}
